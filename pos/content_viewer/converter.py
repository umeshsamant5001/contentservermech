...
def m4v_to_mp4(m4v_url, AppName):
    m4v_path = os.getcwd()
    mp4_path = os.getcwd()
    # print("videpo path is", m4v_path, mp4_path)

    if system_os == "Windows":
        m4v_path = os.path.join(m4v_path, r'storage'+'\\'+str(AppName)+'\\'+r'content\videos\m4v')
        mp4_path = os.path.join(mp4_path, r'storage'+'\\'+str(AppName)+'\\'+r'content\videos\mp4')
        ...
        m4v_path = os.path.join(m4v_path, 'storage/'+str(AppName)+'/content/videos/m4v')
        # print("m4v_path ", m4v_path)
        mp4_path = os.path.join(mp4_path, 'storage/'+str(AppName)+'/content/videos/mp4')
        m4v_filename = m4v_url.split('/')[-1]
...
def wav_to_mp3(wav_url, AppName):
    wav_path = os.getcwd()
    mp3_path = os.getcwd()
    # print(wav_path, mp3_path)

    if system_os == "Windows":
        wav_path = os.path.join(wav_path, r'storage'+'\\'+str(AppName)+'\\'+r'content\audios\wav')
        mp3_path = os.path.join(mp3_path, r'storage'+'\\'+str(AppName)+'\\'+r'content\audios\mp3')
        ...
        wav_path = os.path.join(wav_path, 'storage'+'/'+str(AppName)+'/'+'content/audios/wav')
        mp3_path = os.path.join(mp3_path, 'storage'+'/'+str(AppName)+'/'+'content/audios/mp3')
        wav_filename = wav_url.split('/')[-1]
        wav_path = os.path.join(wav_path, wav_filename)
        mp3_filename = wav_filename.replace('.wav', '.mp3')
        mp3_path = os.path.join(mp3_path, mp3_filename)
